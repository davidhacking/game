...
class ClientTrader(IClientTrader):
    ...
    def _switch_left_menus(self, path, sleep=0.2):
        ...
        try:
            ...
            logger.error("Failed to switch left menu to %s: %s", path, ex)
            # 尝试刷新窗口后重试一次
            self.wait(1)
            try:
                # 清除缓存，强制重新获取菜单句柄
                self._get_left_menus_handle.cache_clear()
                self._get_left_menus_handle().get_item(path).select()
                self._app.top_window().type_keys('{F5}')
                self.wait(sleep)
            except Exception as retry_ex:
                logger.error("Retry failed to switch left menu: %s", retry_ex)
                raise
    ...
    def _get_left_menus_handle(self):
        ...
        while True:
            try:
                ...
                handle.wait("ready", 5)  # 增加等待时间从2秒到5秒
                return handle
            # pylint: disable=broad-except
            except Exception as ex:
                logger.exception("error occurred when trying to get left menus, retrying... (remaining: %d)", count)
                if count <= 0:
                    logger.error("failed to get left menus after all retries")
                    raise
                self.wait(1)  # 重试前等待1秒
            count = count - 1
